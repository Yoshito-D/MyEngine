...
/// @brief クリアシーン
class ClearScene : public GameEngine::BaseScene {
public:
   ClearScene();
...
private:
   ...
   float autoTransitionTime_ = 3.0f;  // 3秒後に自動遷移
   bool hasRequestedTransition_ = false;
   
   SelectionState currentSelection_ = SelectionState::Right;  // デフォルトは右（タイトル）
   
   // スプライト
   std::unique_ptr<GameEngine::Sprite> topSprite_ = nullptr;       // 上のスプライト
   std::unique_ptr<GameEngine::Sprite> leftSprite_ = nullptr;      // 左下のスプライト（ゲーム）
